...
class VisualizationDemo(object):
    def __init__(self, cfg, instance_mode=ColorMode.SEGMENTATION, parallel=False):
        ...
        random.seed(42)
        random.shuffle(clrs)
        clrs = clrs[:len(self.metadata.thing_classes)]
        self.metadata.set(thing_colors=[colors.to_rgb(c=clr) for clr in clrs])
    ...
    def run_on_video(self, video, fontsize=None):
        """
        ...
        def process_predictions(frame, predictions):
            ...
            if "panoptic_seg" in predictions:
                ...
                vis_frame = video_visualizer.draw_instance_predictions(frame,
                                                                       predictions,
                                                                       cols=self.metadata.get("thing_colors"),
                                                                       fontsize=fontsize)
